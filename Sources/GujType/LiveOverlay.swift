...
/// Non-activating bottom-centred heads-up display for the active push-to-talk session.
        ...
        HStack(spacing: 13) {
            HStack(spacing: 4) {
                ForEach(0..<5, id: \.self) { index in
                    Circle().fill(app.isRecording ? Color.white : Color.white.opacity(0.5))
                        .frame(width: app.isRecording ? CGFloat(5 + (index % 2) * 3) : 5, height: app.isRecording ? CGFloat(5 + (index % 2) * 3) : 5)
                }
            }.frame(width: 46)
