...
struct MenuBarView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack { Label(app.activeProviderName, systemImage: app.usesSarvamForCurrentMode ? "cloud.fill" : "laptopcomputer"); Spacer(); Text(app.outputMode.rawValue) }.font(.caption).foregroundStyle(.secondary)
...
private struct DictionaryPage: View { @EnvironmentObject var app: AppState; @State private var term = ""
    var body: some View { VStack(alignment: .leading, spacing: 24) { Text("Personal dictionary").font(.system(size: 31, weight: .bold)); Text("Names, companies and jargon Vaani should spell your way.").foregroundStyle(.secondary); HStack { TextField("e.g. Akash Patel, Vaani, Ahmedabad", text: $term).textFieldStyle(.roundedBorder); Button("Add term") { app.addDictionaryTerm(term); term = "" }.buttonStyle(.borderedProminent).tint(brand).disabled(term.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty) }; Text("Saved on this Mac. When Sarvam is active, these become recognition hints.").font(.caption).foregroundStyle(.secondary); if app.personalDictionary.isEmpty { EmptyState(icon: "text.badge.plus", title: "Make Vaani yours", message: "Add people, businesses, places, and product terms you say often.") } else { FlowLayout(items: app.personalDictionary) { word in HStack(spacing: 7) { Text(word); Button { app.removeDictionaryTerm(word) } label: { Image(systemName: "xmark.circle.fill") }.buttonStyle(.borderless).foregroundStyle(.secondary) }.padding(.horizontal, 11).padding(.vertical, 8).background(.quaternary, in: Capsule()) } }; Spacer() } }
}

private struct SettingsPage: View { @EnvironmentObject var app: AppState; @State private var sarvamKey = ""; @State private var openAIKey = ""
    var body: some View { ScrollView { VStack(alignment: .leading, spacing: 25) { Text("Settings").font(.system(size: 31, weight: .bold)); SettingsCard(title: "Dictation shortcut", subtitle: "Hold to talk; release to finish transcription.") { Picker("Shortcut", selection: $app.shortcut) { ForEach(Hotkey.known) { Text($0.displayName).tag($0) } }.labelsHidden().frame(width: 160) }; SettingsCard(title: "Language & output", subtitle: "Choose how the next dictation is written.") { Picker("Output", selection: $app.outputMode) { ForEach(OutputMode.allCases) { Text($0.rawValue).tag($0) } }.labelsHidden().frame(width: 210) }; SettingsCard(title: "Transcription provider", subtitle: app.outputMode == .english ? "English always uses local Whisper for zero cloud cost." : "Local keeps audio on your Mac. Sarvam uploads completed recordings.") { Picker("Provider", selection: $app.transcriptionProvider) { ForEach(TranscriptionProvider.allCases) { Text($0.rawValue).tag($0) } }.labelsHidden().frame(width: 190) }; SettingsCard(title: "Live preview (beta)", subtitle: "Streams Gujarati and Hindi audio to Sarvam while you hold the shortcut, showing partial words in Vaani. It uses the same audio-based Sarvam billing; keep it off to use the completed-recording flow.") { Toggle("Enable live preview", isOn: $app.useLivePreview).toggleStyle(.switch).disabled(!app.usesSarvamForCurrentMode) }; SettingsCard(title: "Type while speaking (beta)", subtitle: "Writes partial words into the focused app and revises only their changing suffix. Enable Live preview first; some apps may handle synthetic edits differently.") { Toggle("Enable live typing", isOn: $app.typeWhileSpeaking).toggleStyle(.switch).disabled(!app.useLivePreview || !app.usesSarvamForCurrentMode) }; SettingsCard(title: "Permissions", subtitle: "Microphone records speech. Accessibility enables global typing.") { HStack { Button(app.microphonePermission == .granted ? "Microphone ready" : "Allow microphone") { app.requestMicrophone() }.disabled(app.microphonePermission == .granted); Button(app.accessibilityPermission == .granted ? "Accessibility ready" : "Allow accessibility") { app.requestAccessibility() }.disabled(app.accessibilityPermission == .granted) } }; SettingsCard(title: "Sarvam API key", subtitle: "Required only for Gujarati, Gujlish, Hindi, and Hinglish with Sarvam.") { HStack { SecureField("API key", text: $sarvamKey).textFieldStyle(.roundedBorder).frame(width: 210); Button("Save") { app.saveSarvamKey(sarvamKey); sarvamKey = "" }.disabled(sarvamKey.isEmpty) } }; SettingsCard(title: "Optional text cleanup", subtitle: "Sends transcript text only to GPT-5.6 Luna; audio is never uploaded to OpenAI.") { VStack(alignment: .trailing) { Toggle("Use cleanup", isOn: $app.useCloudCleanup).toggleStyle(.switch); HStack { SecureField("OpenAI API key", text: $openAIKey).textFieldStyle(.roundedBorder).frame(width: 210); Button("Save") { app.saveOpenAIKey(openAIKey); openAIKey = "" }.disabled(openAIKey.isEmpty) } } } } } }
}
